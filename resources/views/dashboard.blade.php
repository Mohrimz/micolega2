                    ...
                    <div x-data="{ showSuccess: false, successMessage: '' }" class="mt-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($tutors as $tutor)
                                <!-- Move x-data inside the loop -->
                                <div x-data="{ showGroupSessionModal: false, groupSessionData: { tutorId: {{ $tutor->id }}, skillId: null, availabilityId: null, reason: '', memberEmails: '' } }" class="p-4 border border-gray-300 rounded-lg shadow-sm bg-white">
                                    ...
                                    ">
                                        ...
                                        <div class="flex space-x-4 mt-4">
                                            ...
                                            <!-- Request Group Session Button -->
                                            <button 
                                                type="button" 
                                                @click="showGroupSessionModal = true" 
                                                class="bg-blue-500 text-white px-4 py-2 rounded-md hover:bg-blue-600 w-full">
                                                Request Group Sessions
                                            </button>
                                        </div>
                                    </form>

                                    <!-- Group Session Modal -->
                                    <div x-show="showGroupSessionModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
                                        <div class="bg-white p-8 rounded-lg w-full max-w-md">
                                            <h2 class="text-xl font-bold mb-4">Request Group Session</h2>
                                            <form method="POST" action="{{ route('tutors.requestGroupSession') }}" @submit.prevent="
    $el.submit();
...
">
    ...
    <!-- Reason for Session -->
    <div class="mb-4">
        <label for="reason" class="block text-gray-700">Reason for Session:</label>
        <textarea 
            name="reason" 
            id="reason" 
            x-model="groupSessionData.reason" 
    ...
    <!-- Member Emails -->
    <div class="mb-4">
        <label for="member_emails" class="block text-gray-700">Enter Member Emails (comma-separated):</label>
        <textarea 
            name="member_emails" 
            id="member_emails" 
            x-model="groupSessionData.memberEmails" 
            class="w-full border border-gray-300 rounded-md p-2" 
            required></textarea>
    </div>
    ...
    <div class="flex justify-end space-x-3">
        <button 
            type="button" 
            @click="showGroupSessionModal = false" 
